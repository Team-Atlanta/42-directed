...
echo "Analyzer build completed."
popd

echo "Building klaus components..."
pushd $SRC/klaus/

echo "Building llvm_bitcode_writer..."
pushd ./llvm_bitcode_writer
make -j$(nproc)
cp ./writebc.so /usr/local/lib/
echo "llvm_bitcode_writer build completed and installed to /usr/local/lib/"
popd

echo "Building compiler_wrapper..."
pushd ./compiler_wrapper
./build.sh
echo "compiler_wrapper build completed."

echo "Installing compiler wrappers to /usr/local/bin/..."
cp ./sancc /usr/local/bin/
cp ./san-clang /usr/local/bin/
cp ./san-clang++ /usr/local/bin/
cp ./san-gcc /usr/local/bin/
cp ./san-g++ /usr/local/bin/
echo "All compiler wrappers installed successfully."

popd

echo "Klaus build and installation completed."
popd
